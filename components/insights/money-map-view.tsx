...
export function MoneyMapView({ stateFinance, partyTotals, topRaised, topSpent, stateDominantParty }: MoneyMapViewProps) {
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <div className="mb-2 flex items-center gap-2 text-[12px] text-[var(--poli-faint)]">
        <Link href="/insights" className="hover:text-[var(--poli-text)]">Insights</Link>
        <span>/</span>
        <span>Money Map</span>
      </div>
          ...
          {sortedParties.map(([party, totals]) => {
            ...
            return (
              <div key={party}>
                ...
                <div className="h-2.5 overflow-hidden rounded-full bg-[var(--poli-border)]">
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${pct}%`, backgroundColor: color }}
                  />
                </div>
                <div className="mt-0.5 flex gap-4 text-[10px] text-[var(--poli-faint)]">
          ...
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-[12px] font-medium transition-colors ${
              metric === m
                ...
                : 'border border-[var(--poli-border)] text-[var(--poli-sub)] hover:border-[var(--poli-text)] hover:text-[var(--poli-text)]'
            }`}
          >
            {METRIC_LABELS[m]}
          </button>
      ...
      <div className="rounded-lg border border-[var(--poli-border)] p-4">
        <USMap
          ...
          legend={[
            ...
            { color: 'var(--poli-hover)', label: 'No data' },
          ]}
        />
        <p className="mt-2 text-center text-[12px] text-[var(--poli-faint)]">
        ...
        <div className="mt-6 rounded-lg border border-[var(--poli-border)] p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-full p-1 text-[var(--poli-faint)] hover:bg-[var(--poli-hover)] hover:text-[var(--poli-text)]"
            >
          ...
          <div className="mb-5 grid grid-cols-3 gap-4">
            <div>
              <div className="text-[11px] uppercase tracking-[0.08em] text-[var(--poli-faint)]">Raised</div>
              <div className="text-lg font-semibold text-[var(--poli-text)]">{formatMoney(selectedData.raised)}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-[0.08em] text-[var(--poli-faint)]">Spent</div>
              <div className="text-lg font-semibold text-[var(--poli-text)]">{formatMoney(selectedData.spent)}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-[0.08em] text-[var(--poli-faint)]">Cash on Hand</div>
              <div className="text-lg font-semibold text-[var(--poli-text)]">{formatMoney(selectedData.cash)}</div>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-3">
                {statePartyBreakdown.map(([party, totals]) => (
                  <div key={party} className="flex items-center gap-1.5 text-[11px]">
                    ...
                    <span className="tabular-nums text-[var(--poli-sub)]">
          ...
          <div className="space-y-1">
            {selectedData.politicians.slice(0, 20).map((p, i) => {
              ...
              return (
                <Link
                  ...
                  className="group flex items-center gap-3 rounded-md border border-[var(--poli-border)] px-3 py-2.5 no-underline transition-colors hover:border-[var(--poli-input-border)] hover:bg-[var(--poli-hover)]"
                >
                  <span className="w-5 text-right text-[11px] tabular-nums text-[var(--poli-faint)]">
                    {i + 1}
                  </span>
                  <PartyIcon party={p.party} size={12} />
                  <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-[var(--poli-text)] group-hover:underline">
                    {p.name}
                  </span>
            ...
            {selectedData.politicians.length > 20 && (
              <p className="pt-1 text-center text-[11px] text-[var(--poli-faint)]">
        ...
        <div className="space-y-1">
          {rankList.map((p, i) => {
            ...
            return (
              ...
              >
                <span className="w-5 text-right text-[12px] font-semibold tabular-nums text-[var(--poli-faint)]">
                  {i + 1}
                </span>
                <PartyIcon party={p.party} size={12} />
                <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-[var(--poli-text)] group-hover:underline">

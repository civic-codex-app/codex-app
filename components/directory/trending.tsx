      ...
      <div className="mb-12">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center gap-3 rounded-lg border border-[var(--codex-border)] p-4"
            >
              <div className="h-11 w-11 flex-shrink-0 animate-pulse rounded-full bg-[var(--codex-hover)]" />
              <div className="flex-1 space-y-2">
                <div className="h-3.5 w-24 animate-pulse rounded bg-[var(--codex-hover)]" />
                <div className="h-3 w-16 animate-pulse rounded bg-[var(--codex-hover)]" />
              </div>
            </div>
          ))}
        </div>
        ...
        {data.map((pol) => {
          ...
          return (
            <Link
              ...
              className="group flex items-center gap-3 rounded-lg border border-[var(--codex-border)] p-4 no-underline transition-all hover:border-[var(--codex-text)]"
            >
              {/* Avatar */}
              <div
                className="h-11 w-11 flex-shrink-0 overflow-hidden rounded-full bg-[var(--codex-card)]"
              ...
              <div className="flex-shrink-0 text-right">
                <div className="text-[14px] font-semibold tabular-nums text-[var(--codex-text)]">
                  {pol.follow_count}
                </div>
                <div className="text-[10px] text-[var(--codex-faint)]">followers</div>
              </div>
            </Link>
          )
        })}

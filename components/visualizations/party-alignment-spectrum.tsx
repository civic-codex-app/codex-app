  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-5 flex flex-wrap items-center gap-2">
        ...
        <span className="ml-3 text-[11px] uppercase tracking-[0.1em] text-[var(--codex-faint)]">Party</span>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-lg border border-[var(--codex-border)] bg-[var(--codex-card)] p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold text-[var(--codex-text)]">Biggest Mavericks</span>
          </div>
          <p className="mb-3 text-[12px] text-[var(--codex-faint)]">These politicians break from their party the most</p>
          <div className="space-y-2">
            {topMavericks.map((p) => (
              <Link key={p.slug} href={`/politicians/${p.slug}`} className="flex items-center gap-2.5 rounded-md px-2 py-1.5 no-underline transition-colors hover:bg-[var(--codex-hover)]">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-medium text-[var(--codex-text)]">{p.name}</div>
        ...
        <div className="rounded-lg border border-[var(--codex-border)] bg-[var(--codex-card)] p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold text-[var(--codex-text)]">Most Party Loyal</span>
          </div>
          <p className="mb-3 text-[12px] text-[var(--codex-faint)]">These politicians almost never break from their party</p>
          <div className="space-y-2">
            {topLoyalists.map((p) => (
              <Link key={p.slug} href={`/politicians/${p.slug}`} className="flex items-center gap-2.5 rounded-md px-2 py-1.5 no-underline transition-colors hover:bg-[var(--codex-hover)]">
                <AvatarImage src={p.imageUrl ?? null} alt={p.name} size={28} fallbackColor={partyColor(p.party)} party={p.party} className="h-7 w-7 rounded-full" />
      ...
      <div className="space-y-2">
        {tieredData.map((tier) => {
          ...
          return (
            <div key={tier.key} className="rounded-lg border border-[var(--codex-border)] bg-[var(--codex-card)]">
              <button
                onClick={() => setExpandedTier(isExpanded ? null : tier.key)}
                className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--codex-hover)]"
              >
                {/* Color indicator */}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[14px] font-semibold text-[var(--codex-text)]">{tier.label}</span>
                    <span className="rounded-full px-2 py-0.5 text-[11px] font-medium" style={{ backgroundColor: `${tier.color}15`, color: tier.color }}>
                      {tier.members.length}
                    </span>
                  </div>
                  <p className="text-[12px] text-[var(--codex-faint)]">{tier.description}</p>
                </div>

                {/* Distribution bar */}
                <div className="hidden w-32 items-center gap-2 sm:flex">
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--codex-border)]">
                    <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: tier.color }} />
                  </div>
                  <span className="text-[11px] font-medium tabular-nums text-[var(--codex-faint)]">{pct}%</span>
                </div>

                {/* Expand arrow */}
                <svg
                  width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--codex-faint)" strokeWidth="2" strokeLinecap="round"
                    ...
                    {showMembers.map((p) => {
                      ...
                      return (
                        <Link
                          ...
                          className="flex items-center gap-2.5 rounded-md px-2 py-1.5 no-underline transition-colors hover:bg-[var(--codex-hover)]"
                        >
                          <AvatarImage src={p.imageUrl ?? null} alt={p.name} size={24} fallbackColor={pColor} party={p.party} className="h-6 w-6 rounded-full" />
                          <span className="min-w-0 flex-1 truncate text-[13px] text-[var(--codex-text)]">{p.name}</span>
                          <div className="flex items-center gap-1">
                            <PartyIcon party={p.party} size={10} />
                            <span className="text-[11px] text-[var(--codex-faint)]">{p.state}</span>
                          </div>
